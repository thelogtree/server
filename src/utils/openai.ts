...
export const OpenAIUtil = {
  ...
  getAreLogsRelatedToMessage: async (gptResponse: string) => {
    const completion = await MyOpenAI.createCompletion({
      model: "text-curie-001",
      prompt: `Your job is to determine if a message generated by a bot is useful. The message that the bot generates will be directed towards a customer support agent, but if the bot cannot come up with anything useful to say to the agent then the bot will say that the user's logs are unrelated to the user's message. If the bot says this, you should say "false". Otherwise you should say "true". Here is the bot's message:\n${gptResponse}`,
      temperature: 0,
      max_tokens: 30,
    });
    const textResult = completion.data.choices[0].text || "";
